...
| iam\_admin\_role\_arns | List of IAM role ARNs (e.g., SSO roles, administrator roles, cross-account roles) that should have administrative access to the KMS keys. Takes precedence over iam\_admin\_group. Aligned with AWS best practices for using roles instead of IAM users. | `list(string)` | ```[ "arn:aws:iam::396296719014:role/aws-reserved/sso.amazonaws.com/eu-central-1/AWSReservedSSO_AWSAdministratorAccess_ff0c063028fb63f2" ]``` | no |
