...
function rotate_backups {
  ...
  versions_json=$(aws --cli-connect-timeout 300 s3api list-object-versions --region ${region} --bucket ${backup_bucket_name})
  ...
  for i in $(seq 0 $delete_count); do
    ...
    aws --cli-connect-timeout 300 s3api delete-object --region ${region} --bucket ${backup_bucket_name} --key "$key" --version-id "$version_id"
  done
  ...
  for dm in $delete_markers; do
    ...
    aws --cli-connect-timeout 300 s3api delete-object --region ${region} --bucket ${backup_bucket_name} --key "$dm_key" --version-id "$dm_version_id"
  done
}
